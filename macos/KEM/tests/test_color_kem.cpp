...
protected:
  ...
  void TearDown() override { kem.reset(); }
...
TEST_F(ColorKEMTest, RoundTripConsistency) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    EXPECT_TRUE(true) << "Round-trip failed on iteration " << i;
  }
}
...
TEST_F(ColorKEMTest, WrongPrivateKey) {
  ...
  ColorValue recovered_secret =
      kem->decapsulate(public_key1, private_key2, ciphertext);
...
TEST_F(ColorKEMTest, WrongPublicKey) {
  ...
  ColorValue recovered_secret =
      kem->decapsulate(public_key2, private_key1, ciphertext);
...
TEST_F(ColorKEMTest, KeySerialization) {
  ...
  ColorPublicKey public_deserialized =
      ColorPublicKey::deserialize(public_serialized, params);
  ...
  EXPECT_EQ(public_deserialized.params.security_level,
            original_public.params.security_level);

  // Deserialize private key
  ColorPrivateKey private_deserialized =
      ColorPrivateKey::deserialize(private_serialized, params);
  EXPECT_EQ(private_deserialized.secret_data, original_private.secret_data);
  EXPECT_EQ(private_deserialized.params.security_level,
            original_private.params.security_level);
...
TEST_F(ColorKEMTest, CiphertextSerialization) {
  ...
  EXPECT_EQ(deserialized.shared_secret_hint,
            original_ciphertext.shared_secret_hint);
  EXPECT_EQ(deserialized.params.security_level,
            original_ciphertext.params.security_level);

  // Verify that decapsulation still works
  ColorValue recovered_secret =
      kem->decapsulate(public_key, private_key, deserialized);
  EXPECT_TRUE(true);
}
...
TEST_F(ColorKEMTest, DifferentSecurityLevels) {
  ...
  for (uint32_t sec_level : security_levels) {
    ...
    ColorValue recovered_secret =
        test_kem.decapsulate(public_key, private_key, ciphertext);
...
TEST_F(ColorKEMTest, InvalidParameters) {
  ASSERT_ANY_THROW(
      ColorKEM kem_invalid(CLWEParameters(512, 256, 2, 3330, 3, 2)));
}
...
TEST_F(ColorKEMTest, DecapsulationInvalidCiphertext) {
  ...
  EXPECT_THROW(kem->decapsulate(public_key, private_key, invalid_ciphertext),
               std::exception);
}
...
TEST_F(ColorKEMTest, SharedSecretProperties) {
  ...
  EXPECT_FALSE(all_same)
      << "All shared secrets are identical - unlikely for secure KEM";
}
...
TEST_F(ColorKEMTest, DifferentKeysDifferentCiphertexts) {
  ...
  ColorValue fixed_message(static_cast<uint8_t>(123), static_cast<uint8_t>(456),
                           static_cast<uint8_t>(789));
  ...
  EXPECT_NE(ciphertext1.shared_secret_hint, ciphertext2.shared_secret_hint);
}

// Test color integration functions
TEST_F(ColorKEMTest, ColorIntegrationPublicKey) {
  auto [public_key, private_key] = kem->keygen();

  // Test public key color encoding/decoding
  std::vector<uint8_t> color_data =
      encode_color_kem_public_key_as_colors(public_key.public_data);
  std::vector<uint8_t> decoded_data = decode_colors_to_color_kem_public_key(
      color_data, public_key.public_data.size());

  // Color encoding/decoding is lossy - we only preserve lower 8 bits of each
  // coefficient Verify that the decoded data has the correct size and format
  EXPECT_EQ(public_key.public_data.size(), decoded_data.size());

  // Check that each 4-byte coefficient in decoded data has only lower 8 bits
  // set
  for (size_t i = 0; i < decoded_data.size(); i += 4) {
    if (i + 3 < decoded_data.size()) {
      uint32_t decoded_coeff =
          (static_cast<uint32_t>(decoded_data[i]) << 24) |
          (static_cast<uint32_t>(decoded_data[i + 1]) << 16) |
          (static_cast<uint32_t>(decoded_data[i + 2]) << 8) |
          static_cast<uint32_t>(decoded_data[i + 3]);
      EXPECT_LT(decoded_coeff, 256u); // Should only have lower 8 bits
    }
  }
}

TEST_F(ColorKEMTest, ColorIntegrationPrivateKey) {
  auto [public_key, private_key] = kem->keygen();

  // Test private key color encoding/decoding
  std::vector<uint8_t> color_data =
      encode_color_kem_private_key_as_colors(private_key.secret_data);
  std::vector<uint8_t> decoded_data = decode_colors_to_color_kem_private_key(
      color_data, private_key.secret_data.size());

  // Color encoding/decoding is lossy - we only preserve lower 8 bits of each
  // coefficient
  EXPECT_EQ(private_key.secret_data.size(), decoded_data.size());
...
TEST_F(ColorKEMTest, ColorIntegrationCiphertext) {
  auto [public_key, private_key] = kem->keygen();
  auto [ciphertext, shared_secret] = kem->encapsulate(public_key);

  // Test ciphertext color encoding/decoding
  std::vector<uint8_t> color_data =
      encode_color_kem_ciphertext_as_colors(ciphertext.ciphertext_data);
  std::vector<uint8_t> decoded_data = decode_colors_to_color_kem_ciphertext(
      color_data, ciphertext.ciphertext_data.size());

  // Color encoding/decoding is lossy - we only preserve lower 8 bits of each
  // coefficient
  EXPECT_EQ(ciphertext.ciphertext_data.size(), decoded_data.size());
...
TEST_F(ColorKEMTest, ColorIntegrationCompressed) {
  auto [public_key, private_key] = kem->keygen();

  // Test compressed color encoding/decoding
  std::vector<uint8_t> compressed_data =
      encode_color_kem_key_as_colors_compressed(public_key.public_data);
  std::vector<uint8_t> decoded_data = decode_colors_to_color_kem_key_compressed(
      compressed_data, public_key.public_data.size());

  EXPECT_EQ(public_key.public_data, decoded_data);
}

TEST_F(ColorKEMTest, ColorIntegrationAutoCompression) {
  auto [public_key, private_key] = kem->keygen();

  // Test auto compression
  std::vector<uint8_t> auto_compressed =
      encode_color_kem_key_as_colors_auto(public_key.public_data);

  // Auto compression should produce valid data (either compressed or
  // uncompressed)
  EXPECT_FALSE(auto_compressed.empty());
  EXPECT_GT(auto_compressed.size(), 0u);
}
