...
TEST_F(ColorKEMTest, ColorIntegrationPublicKey) {
    ...
    std::vector<uint8_t> color_data = encode_color_kem_public_key_as_colors(public_key.public_data);
    std::vector<uint8_t> decoded_data = decode_colors_to_color_kem_public_key(color_data, public_key.public_data.size());

    // Color encoding/decoding is lossy - we only preserve lower 8 bits of each coefficient
    // Verify that the decoded data has the correct size and format
    EXPECT_EQ(public_key.public_data.size(), decoded_data.size());

    // Check that each 4-byte coefficient in decoded data has only lower 8 bits set
    for (size_t i = 0; i < decoded_data.size(); i += 4) {
        if (i + 3 < decoded_data.size()) {
            uint32_t decoded_coeff = (static_cast<uint32_t>(decoded_data[i]) << 24) |
...
TEST_F(ColorKEMTest, ColorIntegrationPrivateKey) {
    ...
    std::vector<uint8_t> color_data = encode_color_kem_private_key_as_colors(private_key.secret_data);
    std::vector<uint8_t> decoded_data = decode_colors_to_color_kem_private_key(color_data, private_key.secret_data.size());
...
TEST_F(ColorKEMTest, ColorIntegrationCiphertext) {
    ...
    std::vector<uint8_t> color_data = encode_color_kem_ciphertext_as_colors(ciphertext.ciphertext_data);
    std::vector<uint8_t> decoded_data = decode_colors_to_color_kem_ciphertext(color_data, ciphertext.ciphertext_data.size());
...
TEST_F(ColorKEMTest, ColorIntegrationCompressed) {
    ...
    std::vector<uint8_t> compressed_data = encode_color_kem_key_as_colors_compressed(public_key.public_data);
    std::vector<uint8_t> decoded_data = decode_colors_to_color_kem_key_compressed(compressed_data, public_key.public_data.size());
...
TEST_F(ColorKEMTest, ColorIntegrationAutoCompression) {
    ...
    std::vector<uint8_t> auto_compressed = encode_color_kem_key_as_colors_auto(public_key.public_data);

    // Auto compression should produce valid data (either compressed or uncompressed)
