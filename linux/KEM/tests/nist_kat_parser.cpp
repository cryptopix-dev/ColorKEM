...
// Download ML-KEM KAT file
std::string NIST_KAT_Downloader::download_mlkem_kat(uint32_t security_level) {
    std::string url;
    if (security_level == 512) {
        url = "https://csrc.nist.gov/CSRC/media/Projects/post-quantum-cryptography/documents/example-files/PQCkemKAT_1984.rsp";
    } else if (security_level == 768) {
        url = "https://csrc.nist.gov/CSRC/media/Projects/post-quantum-cryptography/documents/example-files/PQCkemKAT_3040.rsp";
    } else if (security_level == 1024) {
        url = "https://csrc.nist.gov/CSRC/media/Projects/post-quantum-cryptography/documents/example-files/PQCkemKAT_4096.rsp";
    } else {
        throw std::runtime_error("Invalid security level for ML-KEM KAT");
    }
...
// Parse ML-KEM KAT file
std::vector<MLKEM_KAT_TestVector> NIST_KAT_Downloader::parse_mlkem_kat(const std::string& content) {
    std::vector<MLKEM_KAT_TestVector> vectors;
    std::istringstream iss(content);
    std::string line;

    MLKEM_KAT_TestVector current_vector;
    ...
    while (std::getline(iss, line)) {
        ...
        if (line.find("count = ") == 0) {
            ...
            current_vector = MLKEM_KAT_TestVector();
            ...
            current_vector.ct = hex_to_bytes(line.substr(5));
        } else if (line.find("ss = ") == 0) {
            current_vector.ss = hex_to_bytes(line.substr(5));
        }
    }
