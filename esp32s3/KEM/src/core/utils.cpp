...
#ifdef ESP_PLATFORM
    return malloc(size);  // Standard allocation for ESP
#else
    #ifdef HAVE_AVX2
...
#ifdef ESP_PLATFORM
    free(ptr);  // Standard deallocation for ESP
#else
    #ifdef HAVE_AVX2
        _mm_free(ptr);
    #else
...
template class AVXVector<avx_type>;
...
uint32_t montgomery_reduce_avx(avx_type a, uint32_t q) {
...
void secure_random_bytes(uint8_t* buffer, size_t len) {
    ...
    for (size_t i = 0; i < len; i += 4) {
        ...
        uint8_t* rand_bytes = reinterpret_cast<uint8_t*>(&rand_val);
        size_t copy_len = std::min(size_t(4), len - i);
        memcpy(buffer + i, rand_bytes, copy_len);
    }
#else
