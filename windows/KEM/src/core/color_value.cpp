...
ColorValue ColorValue::mod_add(const ColorValue &other,
                               uint32_t modulus) const {
  uint32_t this_val = to_math_value();
  uint32_t other_val = other.to_math_value();
  uint64_t sum = (uint64_t)this_val + other_val;
  if (modulus == 0) return from_math_value(sum);
  uint32_t result = sum % modulus;
  return from_math_value(result);
}

ColorValue ColorValue::mod_subtract(const ColorValue &other,
                                    uint32_t modulus) const {
  uint32_t this_val = to_math_value();
  uint32_t other_val = other.to_math_value();
  uint64_t diff = (uint64_t)this_val + modulus - other_val;
  if (modulus == 0) return from_math_value(static_cast<uint32_t>(diff));
  uint32_t result = diff % modulus;
  return from_math_value(result);
}

ColorValue ColorValue::mod_multiply(const ColorValue &other,
                                    uint32_t modulus) const {
  uint32_t this_val = to_math_value();
  uint32_t other_val = other.to_math_value();
  if (modulus == 0) return from_math_value(this_val * other_val);
  uint32_t a_mod = this_val % modulus;
  uint32_t b_mod = other_val % modulus;
  uint64_t product = (uint64_t)a_mod * b_mod;
...
ColorValue ColorValue::to_hsv() const {
  ...
  return ColorValue(static_cast<uint8_t>(h / 360.0f * 255),
                    static_cast<uint8_t>(s * 255),
                    static_cast<uint8_t>(v * 255), this->a);
}
...
ColorValue ColorValue::from_hsv() const {
  ...
  return ColorValue(static_cast<uint8_t>((r + m) * 255),
                    static_cast<uint8_t>((g + m) * 255),
                    static_cast<uint8_t>((b + m) * 255), this->a);
}
...
__m512i multiply_colors_avx512(__m512i a, __m512i b) {
  // Placeholder - correct intrinsics logic requires more complex sequence
  return a;
}
...
std::vector<uint8_t> ColorValue::serialize() const {
  std::vector<uint8_t> input = {r, g, b, a};
  sha3_ctx_t ctx;
  shake256_init(&ctx);
  shake_update(&ctx, input.data(), input.size());
  shake_xof(&ctx);
  std::vector<uint8_t> output(32);
  shake_out(&ctx, output.data(), 32);
  return output;
}
